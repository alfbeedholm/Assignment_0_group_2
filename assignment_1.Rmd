...
**Answer:**
We can do this using mutate() function from dplyr and the ifelse() function.
```{r}
data <- data |> 
...
*trial.type*: Trial type is a nominal variables as it holds two different non-numeric options: staircase and experiment. They should therefore be classified as a factor in R.
```{r}
data$trial.type <- factor(data$trial.type)
cat("trial.type is now:", class(data$trial.type))
```
*pas*: PAS is the subjective experience that the person rates. It is ordinal, even though it uses numbers, because it has ordering but without a real zero. 
```{r}
data$trial.type <- factor(data$trial.type)
print(class(data$trial.type))
```
